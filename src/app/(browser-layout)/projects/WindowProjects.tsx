...
export default function WindowProject({ loading }: PageProps) {
  ...
  useEffect(() => {
    // Wait until loading is finished and the element is in the DOM
    if (loading) return;
    ...
    const handleScroll = () => {
      // Calculate which item is currently in view based on scroll position
      const scrollLeft = carousel.scrollLeft;
      const itemWidth = carousel.offsetWidth;
      
      // We divide current scroll by width to get the index
      const index = Math.round(scrollLeft / itemWidth);
      
      const clamped = Math.max(0, Math.min(projects.length - 1, index));
      setActiveButton(clamped);
    };
    ...
    <>
      {loading ? (
        ...
        <>
          <div 
            ref={carouselRef} 
            className="carousel md:w-[70%] lg:w-[90%] overflow-x-auto scroll-smooth"
          >
              ...
              <div key={idx} id={`${idx}`} className="carousel-item w-full">
                <div className="card card-side bg-base-100">
                  ...
                  <div className="card-body w-2/5 flex flex-col h-full bg-base-200">
                    <h1 className="text-center card-title text-3xl underline font-bold mb-4 w-full">
                      {project.name}
                    </h1>
          ...
          <div className="flex w-full justify-center gap-2 py-2">
            {projects.map((_, idx) => (
              <a
                ...
                className={`btn btn-xs hover:btn-success ${
                  activeButton === idx ? '!bg-success !text-success-content' : ''
                }`}
                ref={(el) => {
